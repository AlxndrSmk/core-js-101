...
function isLeapYear(date) {
  const year = +date.toLocaleString().slice(4, 8);
...
function timeSpanToString(startDate, endDate) {
  ...
  const h = Math.trunc(res / 3600000)
    .toString()
    .padStart(2, '0');
  const m = Math.trunc((res / 60000) % 60)
    .toString()
    .padStart(2, '0');
  const s = Math.trunc((res / 1000) % 60)
    .toString()
    .padStart(2, '0');
  const ms = Math.trunc(res % 1000)
    .toString()
    .padStart(3, '0');
  return `${h}:${m}:${s}.${ms}`;
}
...
function angleBetweenClockHands(date) {
  ...
  const m = 6 * date.getUTCMinutes();
  const h = 30 * hours + date.getUTCMinutes() / 2;
